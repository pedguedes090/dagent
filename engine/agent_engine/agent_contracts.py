...
ROLE_CONTRACTS: dict[str, AgentContract] = {
    "planner": AgentContract(
        ...
        memory_scope=["explicit planner context envelope only"],
    ...
    "researcher_context": AgentContract(
        ...
        memory_scope=["explicit repository context envelope from the preceding workflow node"],
    ...
    "coder": AgentContract(
        ...
        memory_scope=["explicit workerContext envelope", "latest review feedback only on a rework edge"],
        tool_scope=["PolicyFileEditorTool", "OpenHands TaskTrackerTool", "optional ContainerTerminalTool when Docker/Podman is available"],
        approval_policy=["Requires governance approval for high-risk tasks before write execution."],
        sandbox_policy="Edits an execution-specific git worktree. If Docker/Podman is available, shell commands run in a network-disabled, capability-dropped container; otherwise shell tools are disabled for the coder and verification uses an allowlisted command runner on an isolated copy. Only reviewed changes merge back.",
    ),
    "tester": AgentContract(
        ...
        sandbox_policy="Runs verification in an ephemeral isolated copy. Docker/Podman is used when available; otherwise only allowlisted host commands may run against the copy, never the source workspace.",
    ),
